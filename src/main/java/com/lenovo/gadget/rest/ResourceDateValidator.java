...
public class ResourceDateValidator {
    ...
    public int validateDaysPrevious(String fieldName, final ChartFactory.PeriodName period, String days, Collection<ErrorCollection> errors) {
        final int numberOfDays;
        try {
            numberOfDays = Integer.valueOf(days);
            if (numberOfDays < 0) {
            	errors.add(new SimpleErrorCollection(fieldName,Reason.valueOf("gadget.common.negative.days")));
            }
        } catch (NumberFormatException e) {
        	errors.add(new SimpleErrorCollection(fieldName,Reason.valueOf("gadget.common.days.nan")));
            return INVALID_DAYS;
        }
    ...
    public ChartFactory.PeriodName validatePeriod(String fieldName, String periodName, Collection<ErrorCollection> errors) {
        try {
            return ChartFactory.PeriodName.valueOf(periodName);
        } catch (IllegalArgumentException e) {
            errors.add(new SimpleErrorCollection(fieldName, Reason.valueOf("gadget.common.invalid.period")));
        }
        return null;
    }
     ...
     */
    void validateDaysAgainstPeriod(String fieldName, ChartFactory.PeriodName period, int days, Collection<ErrorCollection> errors) {
        ...
        if (limitForPeriod < days) {
        	errors.add(new SimpleErrorCollection(fieldName, Reason.valueOf("gadget.common.days.overlimit.for.period")));
        }
    }
}